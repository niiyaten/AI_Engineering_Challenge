...
### Rank 8
- score: 46.947133
- record_id: `generic_chunk_ef3527bfaf2568a4`
- record_type: `generic_chunk`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 蒼泉会 ひがし丘総合病院/05.会議/会議録/会議録_2025-08-05.docx`

```text
lock_index=86 type=paragraph style=Compact -->
**価格帯セグメント定義は**** ****charges**** ****の 0/1/2 ****をそれぞれ低・中・高価格帯として承認する。**

<!-- block_index=87 type=paragraph style=Compact -->
業務利用上はレビュー・分析用セグメントとして扱い、患者個別の自動意思決定には使用しない。

<!-- block_index=88 type=paragraph style=Compact -->
**主要因整理は、****smoker****、****bmi****、****age**** ****と数値相互作用特徴量を中心とする方向で承認する。**

<!-- block_index=89 type=paragraph style=Compact -->
ただし、臨床的因果を示す表現は不可とし、「関連」「傾向」に限定する。

<!-- block_index=90 type=paragraph style=Compact -->
**分析手順の文書化は完了と判断する。**

<!-- block_index=91 type=paragraph style=Compact -->
前処理、評価方法、採用モデル、制約、再実行条件を引継ぎ対象に含める。

<!-- block_index=92 type=paragraph style=Compact -->
**次フェーズは本契約では開始しない。**

<!-- block_index=93 type=paragraph style=Compact -->
追加データ連携、PoC運用、可視化強化は別途見積・発注要否を判断する。

<!-- block_index=94 type=paragraph style=Compact -->
**既存オープンアクションは、完了証跡が確認できたもののみクローズし、未確認分はクローズ後処理として継続する。**

<!-- block_index=95 type=paragraph style=Compact -->
**変更:** M02時点で最終候補の主軸としていた linear_baseline から、最終採用モデルを random_forest に変更した。
**理由:** Accuracy単独ではなく Macro F1 を重視した最終評価で総合判断したため。
**承認根拠:** 2025-07-24 変更管理チェックポイント結果、最終報告資料、M03会議での説明・了承。

<!-- block_index=96 type=paragraph style=Heading 2 -->
## 6. アクションアイテム

<!-- block_index=97 type=paragraph style=Normal -->
※ Due Date は当初期限ではなく、最終報告時点で未了のものについて再設定した延長後の期限を記載している。

<!-- block_index=98 type=table rows=18 cols=5 -->
| ID | Action | Owner | Due Date | Status |
| --- | --- | --- | --- | --- |
| A-M01-01 | キックオフ議事録の配布完了有無を確認し、未配布の場合は本会議議事録とあわせて再送する | 加藤 大輔 | 2025-08-06 | Open |
| A-M01-02 | data\\train.csv とカラム説明書の整合確認メモを正式成果物として格納し、共有完了を通知する | 斎藤 悠斗 | 2025-
```
