# test_027

## Question
恒一会 かえで総合病院の提案書において、スコープ対象外としている項目はいくつありますか。

## Route
document_whole_context

## Generated Answer
スコープ対象外

## Retrieved Records

### Rank 1
- score: 100.283592
- record_id: `pptx_slide_9aa31de738238526`
- record_type: `pptx_slide`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/06.報告書/医療法人社団 恒一会 かえで総合病院_最終報告_old.pptx`

```text
Slide 3
2. プロジェクト目的とスコープ
目的
● train.csv の患者属性および検査値から disease（0/1）を予測する分析基盤と初期予測モデルの整備

● 結果は診断の代替ではなく、業務上の判定支援・導入判断材料として提供
スコープ（対象）
● EDA、前処理方針、特徴量選定
● モデル構築（候補比較）、評価
● 成果物：概要書、計画書、データ理解レポート、中間/最終報告書、議事録、スケジュール表
スコープ外（対象外）
実運用
システム実装
電子カルテ
連携
外部データ
取得
継続運用基盤
（MLOps）
臨床診断の
最終決定
（注）上記は提案書・契約書の記載と整合している。
```

### Rank 2
- score: 100.283592
- record_id: `pptx_slide_a16213ac375f3cc4`
- record_type: `pptx_slide`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/06.報告書/医療法人社団 恒一会 かえで総合病院_最終報告.pptx`

```text
Slide 3
2. プロジェクト目的とスコープ
目的
● train.csv の患者属性および検査値から disease（0/1）を予測する分析基盤と初期予測モデルの整備

● 結果は診断の代替ではなく、業務上の判定支援・導入判断材料として提供
スコープ（対象）
● EDA、前処理方針、特徴量選定
● モデル構築（候補比較）、評価
● 成果物：概要書、計画書、データ理解レポート、中間/最終報告書、議事録、スケジュール表
スコープ外（対象外）
実運用
システム実装
電子カルテ
連携
外部データ
取得
継続運用基盤
（MLOps）
臨床診断の
最終決定
（注）上記は提案書・契約書の記載と整合している。
```

### Rank 3
- score: 96.78008
- record_id: `pptx_slide_806c2de22fbc2914`
- record_type: `pptx_slide`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/00.提案/提案書.pptx`

```text
Slide 6
03
スコープ ─ 対象データ
対象データ
ファイル　data\train.csv
行数　3,500
列数　12
目的変数　disease（0:無、1:有）
粒度　患者単位スナップショット（時系列なし）
Notes: 【備考】
スコープ対象外
✖ 実運用システムへの組込み、API化、電子カルテ連携
✖ 医師の診断を代替する臨床判断ロジックの確定
✖ 外部データ取得、追加アノテーション、他システムとのデータ統合
✖ 本データに存在しないKPIの算出
✖ 本番監視、継続学習、MLOps基盤構築
✖ 別途評価用データを前提とした本番精度保証
✖ 個票レベルでの個人特定、患者追跡、属性拡張
```

### Rank 4
- score: 75.878424
- record_id: `pptx_slide_31bcbb876ab203c8`
- record_type: `pptx_slide`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/06.報告書/医療法人社団 恒一会 かえで総合病院_最終報告_old.pptx`

```text
Slide 16
15. 提案・契約との差分管理（1/2）
| col_1 | col_2 | col_3 | col_4 |
| --- | --- | --- | --- |
| 項目 | 差分 | 変更理由 | 承認トレース |
| 成果物 | なし | 契約定義の成果物を全て納品済 | Contract 第4条 |
| スコープ | モデル種別 進化あり | 線形系（T01-T05）→非線形系（hist_gradient_boosting）へ。スコープ内の分析アプローチ最適化に該当 | M02議事録 (2025-09-16) |
| KPI/受入基準 | なし | 提案段階のKPI指標（ROC-AUC、Accuracy等）を維持 | Proposal KPI定義 Contract 第4.1 |
| 契約形態 支払条件 | なし | T&M、最終一括精算、実績工数に基づく | Contract 第6条 |
商業情報（確定値）
| col_1 | col_2 |
| --- | --- |
| 項目 | 値 |
| 実績工数 | 140時間 |
| 契約時時間単価 | 25,000 JPY/時間 |
| 最終請求金額（税抜） | 3,500,000 JPY |
| 消費税額（10%） | 350,000 JPY |
| 最終請求金額（税込） | 3,850,000 JPY |
請求トリガー：最終成果物の検収完了後
支払期日基準：2025-10-07
計算根拠：Contract 第6条、税率10%
工数記録：乙作成の工数表を根拠として甲へ提出
```

### Rank 5
- score: 75.878424
- record_id: `pptx_slide_0f9e8d7d24538af7`
- record_type: `pptx_slide`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/06.報告書/医療法人社団 恒一会 かえで総合病院_最終報告.pptx`

```text
Slide 16
15. 提案・契約との差分管理（1/2）
| col_1 | col_2 | col_3 | col_4 |
| --- | --- | --- | --- |
| 項目 | 差分 | 変更理由 | 承認トレース |
| 成果物 | なし | 契約定義の成果物を全て納品済 | Contract 第4条 |
| スコープ | モデル種別 進化あり | 線形系（T01-T05）→非線形系（hist_gradient_boosting）へ。スコープ内の分析アプローチ最適化に該当 | M02議事録 (2025-09-16) |
| KPI/受入基準 | なし | 提案段階のKPI指標（ROC-AUC、Accuracy等）を維持 | Proposal KPI定義 Contract 第4.1 |
| 契約形態 支払条件 | なし | T&M、最終一括精算、実績工数に基づく | Contract 第6条 |
商業情報（確定値）
| col_1 | col_2 |
| --- | --- |
| 項目 | 値 |
| 実績工数 | 140時間 |
| 契約時時間単価 | 25,000 JPY/時間 |
| 最終請求金額（税抜） | 3,500,000 JPY |
| 消費税額（10%） | 350,000 JPY |
| 最終請求金額（税込） | 3,850,000 JPY |
請求トリガー：最終成果物の検収完了後
支払期日基準：2025-10-07
計算根拠：Contract 第6条、税率10%
工数記録：乙作成の工数表を根拠として甲へ提出
```

### Rank 6
- score: 75.254251
- record_id: `generic_chunk_06c339eee5202c19`
- record_type: `generic_chunk`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/05.会議/報告資料/報告資料_2025-09-16.docx`

```text
yle=Compact -->
T03: 0.7126899909960438

<!-- block_index=27 type=paragraph style=Compact -->
T05: 0.7126899909960438

<!-- block_index=28 type=paragraph style=Compact -->
run-level 指標（analysis.metrics より）:

<!-- block_index=29 type=paragraph style=Compact -->
auc_roc: 0.8250532501536466

<!-- block_index=30 type=paragraph style=Compact -->
precision_at_top10pct: 0.9428571428571428

<!-- block_index=31 type=paragraph style=Compact -->
brier_score: 0.17514583544772114

<!-- block_index=32 type=paragraph style=Compact -->
selected_feature_count: 10, excluded_feature_count: 1

<!-- block_index=33 type=paragraph style=Compact -->
実装／環境

<!-- block_index=34 type=paragraph style=Compact -->
実験は線形系（linear_baseline 系）モデル群で実施。decision-tuning（クラス判定重みの調整）が T04 の改善要因として報告されています（visible_trials の change_summary に記載）。

<!-- block_index=35 type=paragraph style=Heading 2 -->
## 3. 主要な分析結果

<!-- block_index=36 type=paragraph style=Compact -->
モデル比較（可視領域の要点）

<!-- block_index=37 type=paragraph style=Compact -->
ベースライン（T01）から段階的にクラスバランスや特徴量拡張、閾値調整を行い、T04（閾値チューニング済み線形モデル）が最も高い f1_macro を示した（f1_macro = 0.7329671168078127）。このことは、現データにおいて「モデル構造を大きく変えずに、決定閾値やクラス判断の調整で性能改善が得られる」ことを示唆します。

<!-- block_index=38 type=paragraph style=Compact -->
AUC-ROC（0.8250532501536466）や top10% precision（0.9428571428571428）が比較的良好である点は、スコア上位の予測が高い精度で陽性を含む可能性を示しており、閾値運用による業務ルール設計の余地があります。

<!-- block_index=39 type=paragraph style=Compact -->
特徴量・前処理の状況

<!-- block_index=40 type=paragraph style=Compact -->
モデルが使用している選択特徴数は 10、除外特徴は 1（analysis.metrics / run_summary 相関）で、feature selection が適用されていることを確認しています。

<!-- block_in
```

### Rank 7
- score: 72.256511
- record_id: `pptx_slide_cd243a0fb19bbbd8`
- record_type: `pptx_slide`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/00.提案/提案書.pptx`

```text
Slide 2
目次
01
背景
02
目的
03
スコープ
04
分析アプローチ
05
実施体制
06
スケジュール案
07
成果物定義
08
費用見積
09
リスクと対応策
10
前提条件
11
次アクション
```

### Rank 8
- score: 71.532223
- record_id: `python_code_chunk_ba094e35462ae0cf`
- record_type: `python_code_chunk`
- source_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/04.分析/analysis_project/src/__init__.py`

```text
# Python Source: __init__.py

## Source
- raw_path: `share/共有ドライブ/プロジェクト/医療法人社団 恒一会 かえで総合病院/04.分析/analysis_project/src/__init__.py`
- source_sha1: `da39a3ee5e6b4b0d3255bfef95601890afd80709`
- line_count: 0
- function_count: 0
- class_count: 0
- has_main_guard: False

## Imports

該当データはありません。

凡例: `module` はimport元、`name` はfrom importの対象、`asname` は別名を表します。

## Functions

該当データはありません。

凡例: `name` は関数名、`lineno` は開始行、`args` は引数、`docstring` は先頭説明を表します。

## File Operations

該当データはありません。

凡例: `call` は呼び出し名、`string_args` と `string_keywords` はファイルパス候補を含む文字列引数です。

## Code

```python

```
```
